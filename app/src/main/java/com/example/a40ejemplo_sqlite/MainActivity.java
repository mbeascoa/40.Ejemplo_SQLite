...
public class MainActivity extends AppCompatActivity {
    ...
    public void recuperarDatos(View view) {
        try {
            String[] args = new String[]{"Usuario creado 1"};
            String codigo = "", nombre = "", contrasenia = "";
            BaseDatosHelper usdbh = new BaseDatosHelper(this, "DBUsuarios", null, 1);

            SQLiteDatabase db = usdbh.getReadableDatabase();
            Cursor c = db.rawQuery("SELECT codigo,nombre, contrasenia FROM Usuarios where nombre=?", args);
    ...
    // creamos este método compatible con el anterior

    private void consultarDatos(SQLiteDatabase sqlDB) {
        //se genera un array de argumentos para construir el SQL
        ...
        Cursor c = sqlDB.rawQuery(sql, args);

        int IdIndex = c.getColumnIndexOrThrow ("codigo");
    ...
    private void insertarNuevo(SQLiteDatabase sqlDB) {

        ContentValues registroNuevo = new ContentValues();
        registroNuevo.put( codigo,  6);
        registroNuevo.put(nombre, Nombre6);
        registroNuevo.put(contrasenia, contrasenia6);
        sqlDB.insert("Usuarios", null, registroNuevo);
